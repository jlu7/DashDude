...
[CustomEditor(typeof(tk2dCurvedTextMesh))]
class tk2dCurvedTextMeshEditor : Editor
{
	...
	tk2dCurvedTextMesh[] targetTextMeshes = new tk2dCurvedTextMesh[0];
#if !(UNITY_3_5 || UNITY_4_0 || UNITY_4_0_1 || UNITY_4_1 || UNITY_4_2)
	Renderer[] renderers = new Renderer[0];
#endif

	void OnEnable() {
		targetTextMeshes = new tk2dCurvedTextMesh[targets.Length];
		for (int i = 0; i < targets.Length; ++i) {
			targetTextMeshes[i] = targets[i] as tk2dCurvedTextMesh;
		}
	...
	{
		tk2dCurvedTextMesh textMesh = (tk2dCurvedTextMesh)target;
	...
	void UndoableAction( System.Action<tk2dCurvedTextMesh> action ) {
		tk2dUndo.RecordObjects (targetTextMeshes, "Inspector");
		foreach (tk2dCurvedTextMesh tm in targetTextMeshes) {
			action(tm);
		}
	}
    ...
    public override void OnInspectorGUI()
    {
		...
		if (allFonts != null && allFonts.Length > 0)
        {
            ...
            float newCurveWidth = EditorGUILayout.FloatField("Curve Width", textMesh.curveWidth);
            if (newCurveWidth != textMesh.curveWidth) UndoableAction(tm => tm.curveWidth = newCurveWidth);

            float newCurveStrength = EditorGUILayout.FloatField("Curve Strength", textMesh.curveStrength);
            if (newCurveStrength != textMesh.curveStrength) UndoableAction(tm => tm.curveStrength = newCurveStrength);
			...
			if (sortingOrder != targetTextMeshes[0].SortingOrder) {
            	...
            	foreach (tk2dCurvedTextMesh s in targetTextMeshes) {
				...
				if (drawGradientScroller)
				{
					...
					if (GUIUtility.hotControl == 0 && Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
					{
						...
						if (newTextureGradient != textMesh.textureGradient) {
							UndoableAction( delegate(tk2dCurvedTextMesh tm) {
    ...
    static void DoCreateTextMesh()
    {
        ...
        tk2dCurvedTextMesh dupeMesh = GameObject.FindObjectOfType(typeof(tk2dCurvedTextMesh)) as tk2dCurvedTextMesh;
		if (dupeMesh) 
        ...
        tk2dCurvedTextMesh textMesh = go.AddComponent<tk2dCurvedTextMesh>();
