...
[System.Serializable]
public class tk2dCurvedTextMeshData
{
...
[AddComponentMenu("2D Toolkit/Text/tk2dCurvedTextMesh")]
/// <summary>
/// Text mesh
/// </summary>
public class tk2dCurvedTextMesh : MonoBehaviour, tk2dRuntime.ISpriteCollectionForceBuild
{
    ...
    [SerializeField]
    tk2dCurvedTextMeshData data = new tk2dCurvedTextMeshData();
    ...
    public void Clone(tk2dTextMesh cloneFrom)
    {
        if (cloneFrom != null)
        {
            this.text = cloneFrom.text;
            this.wordWrapWidth = cloneFrom.wordWrapWidth;
            this.textureGradient = cloneFrom.textureGradient;
            this.color = cloneFrom.color;
            this.color2 = cloneFrom.color2;
            this.Commit();
        }
    }
    ...
    void UpgradeData()
    {
        if (data.version != 1)
        {
            ...
            data.curveWidth = _curveWidth;
            data.curveStrength = _curveStrength;
    ...
    /// <summary>Gives the textfield a circular curve. Call <see cref="Commit"/> to commit changes.</summary>
    public float curveWidth { get { UpgradeData(); return data.curveWidth; } set { UpgradeData(); if (data.curveWidth != value) { data.curveWidth = value; SetNeedUpdate(UpdateFlags.UpdateText); } } }
    /// <summary>Gives the textfield a circular curve. Call <see cref="Commit"/> to commit changes.</summary>
    public float curveStrength { get { UpgradeData(); return data.curveStrength; } set { UpgradeData(); if (data.curveStrength != value) { data.curveStrength = value; SetNeedUpdate(UpdateFlags.UpdateText); } } }
    ...
    {
        return tk2dCurvedTextGeomGen.GetMeshDimensionsForString(str, tk2dCurvedTextGeomGen.Data(data, _fontInst, _formattedText));
    }
    ...
    {
        tk2dCurvedTextGeomGen.CurvedGeomData geomData = tk2dCurvedTextGeomGen.Data(data, _fontInst, _formattedText);
        Vector2 dims = tk2dCurvedTextGeomGen.GetMeshDimensionsForString(FormatText(str), geomData);
        float offsetY = tk2dCurvedTextGeomGen.GetYAnchorForHeight(dims.y, geomData);
        float offsetX = tk2dCurvedTextGeomGen.GetXAnchorForWidth(dims.x, geomData);
    ...
    public void Init()
    {
        if (_fontInst && ((updateFlags & UpdateFlags.UpdateBuffers) != 0 || mesh == null))
        {
            ...
            var geomData = tk2dCurvedTextGeomGen.Data(data, _fontInst, _formattedText);

            // volatile data
            int numVertices;
            int numIndices;
            tk2dCurvedTextGeomGen.GetTextMeshGeomDesc(out numVertices, out numIndices, geomData);
            ...
            int target = tk2dCurvedTextGeomGen.SetTextMeshGeom(vertices, uvs, uv2, untintedColors, 0, geomData, curveWidth, curveStrength);
            ...
            tk2dCurvedTextGeomGen.SetTextMeshIndices(triangles, 0, 0, geomData, target);
    ...
    public virtual void DoNotUse__CommitInternal()
    {
