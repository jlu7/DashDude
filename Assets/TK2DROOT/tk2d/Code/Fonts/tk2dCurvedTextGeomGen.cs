...
public static class tk2dCurvedTextGeomGen
{
	public class CurvedGeomData
	{
		internal tk2dCurvedTextMeshData textMeshData = null;
		internal tk2dFontData fontInst = null;
		internal string formattedText = "";
	}

	// Use this to get a correctly set up textCurvedGeomData object
	// This uses a static global tmpData object and is not thread safe
	// Fortunately for us, neither is the rest of Unity.
	public static CurvedGeomData Data(tk2dCurvedTextMeshData textMeshData, tk2dFontData fontData, string formattedText) {
		tmpData.textMeshData = textMeshData;
		tmpData.fontInst = fontData;
		tmpData.formattedText = formattedText;
		return tmpData;
	}
	private static CurvedGeomData tmpData = new CurvedGeomData();
    ...
    static void ApplyTextCurve(Vector3[] pos,int offset, int targetStart, int targetEnd, float curveWidth, float curveStrength)
    {
        for (int i = targetStart * 4; i < targetEnd * 4; ++i)
        {
            Vector3 v = pos[offset + i];

            float t = v.x / curveWidth;
            if (t > 1 || t < -1) t = 0;
            float circY = curveWidth / curveStrength * Mathf.Sin(t * Mathf.PI);
            if (curveWidth < 0)
            {
                v.y -= circY;
            }else
