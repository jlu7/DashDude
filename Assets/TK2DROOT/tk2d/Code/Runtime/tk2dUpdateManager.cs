...
public class tk2dUpdateManager : MonoBehaviour {
	...
	void FlushQueuesInternal() {
        ...
        count = curvedTextMeshes.Count;
        for (int i = 0; i < count; ++i)
        {
            tk2dCurvedTextMesh tm = curvedTextMeshes[i];
            if (tm != null)
            {
                tm.DoNotUse__CommitInternal();
            }
        }
        curvedTextMeshes.Clear();
	}
    ...
    [SerializeField]
    List<tk2dCurvedTextMesh> curvedTextMeshes = new List<tk2dCurvedTextMesh>(64);
}
